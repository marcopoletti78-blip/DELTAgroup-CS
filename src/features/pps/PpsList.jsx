// Lista delle PPS salvate su Supabase — modulo autonomo.
...
export default function PpsList({ onNew, onOpen, onBack, isMobile = false }) {
  ...
  const doDelete = async (e, id) => {
    e.stopPropagation();
    if (!window.confirm("Eliminare definitivamente questa PPS?")) return;
    setDeletingId(id); setErr(null);
    const { error } = await supabase.from("pps").delete().eq("id", id);
    setDeletingId(null);
    if (error) {
      ...
      <div style={{ maxWidth: "1000px", margin: "0 auto" }}>
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: "20px", flexWrap: "wrap", gap: "12px" }}>
          <div>
            ...
            <h1 style={{ ...SERIF, fontSize: "26px", fontWeight: 700, color: N, margin: "4px 0 0" }}>PPS — Archivio</h1>
          </div>
                ...
                <tbody>
                  {rows.map((p) => (
                    <tr key={p.id} onClick={() => onOpen(p.id)} style={{ cursor: "pointer" }}
                      ...
                      <td style={{ ...TD, fontWeight: 700, color: AC }}>{p.codice || "—"}</td>
                      <td style={TD}>{p.cliente || "—"}</td>
                      <td style={TD}>{p.luogo || "—"}</td>
                      <td style={TD}>{p.tipo_servizio || "—"}</td>
                      <td style={TD}><StatoBadge stato={p.stato} /></td>
                      <td style={{ ...TD, textAlign: "center" }}>{p.versione ?? "—"}</td>
                      <td style={{ ...TD, color: TM, whiteSpace: "nowrap" }}>{formatDate(p.updated_at)}</td>
                      <td style={{ ...TD, textAlign: "right" }}>
                        <button
                          onClick={(e) => doDelete(e, p.id)}
                          disabled={deletingId === p.id}
                          title="Elimina"
                          style={{ ...SANS, background: "none", border: "none", cursor: deletingId === p.id ? "wait" : "pointer", fontSize: "16px", color: ERR, padding: "4px 8px", borderRadius: "6px" }}>
                          {deletingId === p.id ? "…" : "🗑"}
                        </button>
                      </td>
                    </tr>
